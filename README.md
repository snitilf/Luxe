...
- **Trust model** - Artifact JavaScript is trusted to the level of the agent that authored the artifact: through the documented `window.luxe` API it can queue feedback and send it immediately, without you pressing **Send to Agent**, and every send carries the page outline described above.
  That is a deliberate capability, because it is what lets an artifact ask you a question in-page and return your answer straight away.
...
- **Framing and cross-origin writes** - The session page refuses to be framed at all (`X-Frame-Options: DENY` and `frame-ancestors 'none'`), and artifact routes allow only same-origin framing (`frame-ancestors 'self'`) so Luxe's own chrome can host the artifact iframe. No other CSP is applied to artifact content, so artifacts render exactly as authored.
  State-changing routes - `/shutdown`, `/api/sessions`, `/api/end`, `/api/:key/end`, `/api/:key/prompts`, `/api/:key/agent-reply`, `/api/:key/layout-warnings`, and the whiteboard writes - reject (`403`) any request a browser attached a foreign `Origin` or `Referer` to, including the `Origin: null` a sandboxed iframe sends.
