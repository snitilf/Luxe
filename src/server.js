...
// `luxe <file>` invocation re-spawns a fresh server and adopts resumable sessions from
// state.json. Browser-ended sessions still require the explicit --reopen opt-in. Set
// LUXE_IDLE_TIMEOUT_MS to 0/off to disable, or to a custom millisecond budget.
export function resolveIdleTimeoutMs(env = process.env) {
  const raw = env.LUXE_IDLE_TIMEOUT_MS?.trim();
...
export async function serve({
  ...
  const verbose = debug || process.env.LUXE_DEBUG === "1";
  ...
  // (LUXE_HOST) or naming a link host (LUXE_LINK_HOST) adds that host,
  ...
  // LUXE_ALLOWED_HOSTS; a lone "*" there disables the guard for operators
  ...
  app.get("/health", (req, res) => {
    res.json({ ok: true, app: "luxe", version });
  });
  ...
  app.post("/api/sessions", async (req, res, next) => {
    try {
      ...
      // `luxe <file>` is the exact behavior this route exists to prevent - require an
      // explicit `reopen` opt-in instead of reviving it automatically. Agent-initiated ends
      // (`luxe end`) keep reviving on the next open, same as before this change.
  ...
  app.get("/api/:key/export", async (req, res, next) => {
    try {
      ...
      res.setHeader("x-luxe-export-warning-count", String(unresolved.length));
      res.setHeader("x-luxe-export-notice-count", String(notices.length));
  ...
  app.get(/^\/artifact\/([^/]+)\/index\.html$/, async (req, res, next) => {
    try {
      ...
      res.type("html").send(injectLuxeSdk(html, key));
    } catch (error) {
      next(error);
    }
  });
...
// A lone "*" in LUXE_ALLOWED_HOSTS is an explicit opt-out of the Host
...
// LUXE_ALLOWED_HOSTS. This is an AND check: a client-spoofed forwarded host
...
// Guard the state-changing whiteboard routes against CSRF: a browser attaches an Origin/Referer
...
// `data-luxe-live-reload-root` attribute on its root element or
// `<meta name="luxe-live-reload" content="root">`.
  ...
  try {
    ...
    if (hasLiveReloadRootOptIn(html)) {
      return {
        ...
        options: {
          ...baseOptions,
          ignored: /(^|[/\\])(\.git|node_modules|dist|build|\.luxe)([/\\]|$)/,
        },
      };
    }
  } catch {
...
export function hasLiveReloadRootOptIn(html) {
  ...
  if (/<html\b[^>]*\sdata-luxe-live-reload-root(?:[\s=>/]|$)[^>]*>/i.test(searchableHtml)) return true;
  return /<meta\b(?=[^>]*name=["']luxe-live-reload["'])(?=[^>]*content=["']root["'])[^>]*>/i.test(searchableHtml);
}
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Luxe Whiteboard</title>
<link rel="stylesheet" href="/whiteboard-assets/whiteboard.css">
</head>
<body>
<script>window.__luxeWhiteboardChannelToken=${JSON.stringify(channelToken)};</script>
...
void key;
const deriveQueueKey=${deriveLuxeQueueKey.toString()};
...
(${createArtifactSdk.toString()})(deriveQueueKey, isNativeInteractiveControl, mermaidHelpers, ${JSON.stringify(luxeTokensCss)}, buildDomSnapshot, buildStructuralSelector);
})();`;
}
