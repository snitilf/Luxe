...
// Builds a portable copy of a Luxe artifact by inlining only its LOCAL assets - files on disk
...
// browser loads them at render time, so the exported copy renders correctly wherever
...
export async function buildSelfContainedHtml(html, options = {}) {
  ...
  const ctx = {
    ...
    maxAssetBytes: resolveBytes(
      ...
      process.env.LUXE_EXPORT_MAX_ASSET_BYTES,
      DEFAULT_MAX_ASSET_BYTES,
    ),
    maxBundleBytes: resolveBytes(
      options.maxBundleBytes,
      process.env.LUXE_EXPORT_MAX_BUNDLE_BYTES,
...
async function inlineScript(tag, attrs, body, closeTag, baseDir, ctx) {
  ...
  if (isInjectedLuxeSdkSrc(src)) return "";
  if (isModuleScript(attrs)) {
...
function isInjectedLuxeSdkSrc(src) {
